...
function buildTaskList(folder) {
  ...
  var url = 'http://127.0.0.1:8000/api/folder/task-list/';
  fetch(url, {
    method: 'POST',
    headers: {
...
function deleteItem(item) {
  ...
  fetch(`http://127.0.0.1:8000/api/folder/task-delete/`, {
    method: 'DELETE',
    headers: {
      'Content-type': 'application/json',
      'X-CSRFToken': csrftoken
    },
    body: JSON.stringify({ folderId: currentFolder.id, taskId: item.id })
  }).then((response) => {
...
function createTask() {
  ...
  var url = `http://127.0.0.1:8000/api/folder/task-create/`;
  fetch(url, {
    method: 'POST',
    headers: {
      'Content-type': 'application/json',
    ...
    body: JSON.stringify({ folder: currentFolder.id, title: title })
  }).then((response) => {
    buildTaskList(currentFolder);
  });
}
...
function removeFolder(folderItem) {
  ...
  fetch('http://127.0.0.1:8000/api/folder-delete/', {
    method: 'DELETE',
    headers: {
      'Content-type': 'application/json',
      'X-CSRFToken': csrftoken
...
function checkUncheck(item) {
  ...
  fetch(`http://127.0.0.1:8000/api/folder/task-update/`, {
    method: 'POST',
    headers: {
      'Content-type': 'application/json',
      'X-CSRFToken': csrftoken
